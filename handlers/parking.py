...
async def show_help(message: Message, db, is_approved: bool, is_admin: bool, is_moderator: bool, **kwargs):
    ...
    if is_admin:
        text += (
            ...
            "Модератор может принимать заявки, делать объявления и управлять местами.\n"
            "Для добавления — укажите Telegram ID через запятую в переменной "
            "окружения <code>MODERATOR_IDS</code> на Render (раздел Environment).\n"
            "Например: <code>MODERATOR_IDS=123456789,987654321</code>\n"
